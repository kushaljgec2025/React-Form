...
function Form() {
      ...
      const response = await fetch(
        `https://www.universal-tutorial.com/api/getaccesstoken`,
        {
          ...
          headers: {
            ...
            "api-token":
              "6rEyOY-NUt0jEa7QXKQsox7RnOq7ajgiFvsjVSNahDzenI-fswIi9P1VMvR2t2liwPM",
            "user-email": "kushalghosh510@gmail.com",
          },
        }
      );
  ...
  const validate = () => {
    ...
    if (!/[a-z]/.test(formData.password)) {
      ...
      newErrors.password.lowercase =
        "Password must be contain a lowercase letters";
    }
    if (!/[A-Z]/.test(formData.password)) {
      if (!newErrors.password) {
        newErrors.password = {};
      ...
      newErrors.password.upperCase =
        "Password must be contain a Uppercase letters";
    }
    if (!/[!@#$%^&*]/.test(formData.password)) {
      if (!newErrors.password) {
        newErrors.password = {};
      }
      newErrors.password.specialChar =
        "This field must contain at least one of the special characters: @#$&*%";
    }
      ...
      <form className="form" onSubmit={handleSubmit}>
        ...
        <div className="input-container">
          ...
          {errors.panNo && touched.panNo && (
            <p className="error">{errors.panNo}</p>
          )}
        </div>
        <div className="input-container">
          ...
          {errors.aadharNo && touched.aadharNo && (
            <p className="error">{errors.aadharNo}</p>
          )}
        </div>
        <div>
          <div className="input-container">
          ...
          <div className="password-error-container">
            {errors.password && errors.password.message && touched.password && (
              <p className="error">{errors.password.message}</p>
            )}
            {errors.password && errors.password.number && touched.password && (
              <p className="error">{errors.password.number}</p>
            )}
            {errors.password &&
              errors.password.lowercase &&
              touched.password && (
                <p className="error">{errors.password.lowercase}</p>
              )}
            {errors.password &&
              errors.password.upperCase &&
              touched.password && (
                <p className="error">{errors.password.upperCase}</p>
              )}
            {errors.password &&
              errors.password.specialChar &&
              touched.password && (
                <p className="error">{errors.password.specialChar}</p>
              )}
          </div>
        </div>
        <button
